...
static value
reason_libvterm_Val_screencell(const VTermScreenCell *pScreenCell) {
  CAMLparam0();
  CAMLlocal1(ret);
...
void finalize_sb_line(value v) {
  sb_line *line = Data_custom_val(v);
  free(line->cells);
}

static struct custom_operations sb_line_ops = {"vterm.sb_line",
                                               &finalize_sb_line,
...
int reason_libvterm_onScreenSbPushLineF(int cols, const VTermScreenCell *cells,
  ...
  size_t bytes_n = cols * sizeof(VTermScreenCell);
  VTermScreenCell *ret_cells = malloc(bytes_n);
  memcpy(ret_cells, cells, bytes_n);

  ret = caml_alloc_custom_mem(&sb_line_ops, sizeof(sb_line), 0);
  sb_line *line = Data_custom_val(ret);
  line->cols = cols;
  line->cells = ret_cells;
...
int reason_libvterm_onScreenSbPopLineF(int cols, VTermScreenCell *cells,
  ...
  ret = caml_callback(*reason_libvterm_onScreenSbPopLine, Val_int(user));
  if (Is_none(ret)) {
    CAMLreturn(0);
  }

  sb_line *line = Data_custom_val(Some_val(ret));
  size_t min_cols = cols < line->cols ? cols : line->cols;
  memcpy(cells, line->cells, min_cols * sizeof(VTermScreenCell));

  if (cols > line->cols) {
    VTermScreenCell empty = {0};
    empty.chars[0] = 0;
    empty.width = 1;
    empty.fg.type = VTERM_COLOR_DEFAULT_FG;
    empty.bg.type = VTERM_COLOR_DEFAULT_BG;
